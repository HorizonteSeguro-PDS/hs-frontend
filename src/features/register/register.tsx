...
import { useRegister, useOrganizations, useCreateOrganization } from '@/features/register/hooks'
...
export default function Register() {
  ...
  function handleCreateOrg() {
    ...
    createOrg(newOrgName.trim(), {
      onSuccess: (org) => {
        updateField('organization_id', org.id)
        setNewOrgName('')
        setShowNewOrg(false)
      },
    })
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6 pt-2">
            ...
            <Field label="Organização">
                    ...
                    {filteredOrgs.length === 0 ? (
                      ...
                      filteredOrgs.map((org) => (
                        <li
                          ...
                          className={`cursor-pointer px-4 py-2 text-sm hover:bg-[#f0f9ff] transition-colors ${
                            formData.organization_id === org.id ? 'font-semibold text-[#1FA6A0]' : 'text-[#0a0a0a]'
                          }`}
                        >
                          {org.name}
                        </li>
              ...
              {!showNewOrg ? (
                ...
                <div className="flex flex-col gap-2 rounded-xl border border-[#E5E7EB] bg-[#fafafa] p-3 mt-0.5">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => { setShowNewOrg(false); setNewOrgName('') }}
                      className="text-[#0A0A0A80] hover:text-black">
                      <X size={14} />
                    </button>
                  </div>
                  <input type="text" placeholder="Nome da organização..." value={newOrgName}
                    onInput={(e) => setNewOrgName((e.target as HTMLInputElement).value)}
                    className="input input-bordered input-sm w-full rounded-lg bg-white border-[#0A0A0A80] text-[#0A0A0A80] focus:border-[#1FA6A0] focus:outline-none" />
                  <button type="button" disabled={!newOrgName.trim() || isCreatingOrg} onClick={handleCreateOrg}
                    className="btn btn-sm rounded-lg border-none bg-[linear-gradient(16deg,#1FA6A0_0.1%,#2F7DBB_57.69%,#3555A3_99.4%)] text-white disabled:opacity-60">
                    {isCreatingOrg ? 'Criando...' : 'Criar e selecionar'}
                  </button>
                </div>
              )}
            </Field>
...
function StepDot({ active, done, label }: { active: boolean; done: boolean; label: string }) {
  return (
    <div className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-semibold transition-colors ${
      active
        ? 'bg-[linear-gradient(16deg,#1FA6A0_0.1%,#2F7DBB_57.69%,#3555A3_99.4%)] text-white'
        : done
        ? 'bg-[#1FA6A0] text-white'
