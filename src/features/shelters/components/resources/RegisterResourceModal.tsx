...
import { useInitialStock } from '@/features/shelters/hooks'
...
export default function RegisterResourceModal({ open, shelterId, onClose }: RegisterResourceModalProps) {
  const [formData, setFormData] = useState<FormData>(initialFormData)
  const { mutate: createInitialStock, isPending } = useInitialStock(shelterId)
  ...
  function handleSubmit() {
    ...
    createInitialStock(
      ...
      {
        onSuccess: () => {
          showToast('Recurso cadastrado com sucesso!', 'success')
          handleClose()
        },
        onError: () => showToast('Erro ao cadastrar recurso.', 'error'),
      },
    )
  }
      ...
      <div className="flex w-full max-w-[512px] flex-col items-start [overflow:clip] rounded-[14px] shadow-xl">
        <div className={`flex w-full items-start justify-between rounded-t-[16px] p-4 ${headerGradient}`}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Novo Tipo de Recurso</h2>
              <p className="text-sm text-white/80">Crie um novo tipo e registre a primeira entrada</p>
            </div>
          </div>
        ...
        <div className="flex w-full flex-col gap-5 bg-white p-6">
          <Field label="Categoria" required>
            <Select
              ...
              value={formData.lot_category}
              options={lotCategoryOptions}
              onChange={(v) => update('lot_category', v)}
            />
          </Field>

          <Field label="Nome do Tipo de Recurso" required>
            <Input
          ...
          <Field label="Origem (opcional)">
            <Input
              placeholder="Ex: Doação Igreja X..."
              value={formData.source}
              onInput={(v) => update('source', v)}
            />
          </Field>
        ...
        <div className="flex w-full justify-end gap-3 rounded-b-[14px] bg-white p-4">
          ...
          <button
            ...
            disabled={isPending || !formData.lot_category || !formData.name || !formData.unit || !formData.quantity}
            className={`btn border-none text-white disabled:opacity-60 ${buttonGradient}`}
          >
            {isPending ? 'Cadastrando...' : 'Cadastrar Recurso'}
          </button>
        </div>
      </div>
    </div>
...
function Field({ label, required, children }: { label: string; required?: boolean; children: preact.ComponentChildren }) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-bold text-black">{label}{required ? ' *' : ''}</label>
      {children}
    </div>
  )
}

function Input({ placeholder, value, onInput, type = 'text' }: { placeholder: string; value: string; onInput: (v: string) => void; type?: string }) {
  return (
    <input
      type={type}
...
function Select({ placeholder, value, options, onChange, disabled }: { placeholder: string; value: string; options: { value: string; label: string }[]; onChange: (v: string) => void; disabled?: boolean }) {
  return (
    <select
      value={value}
