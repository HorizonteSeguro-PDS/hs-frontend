...
interface ShelterFilterSortModalProps {
  open: boolean
  onClose: () => void
  onSelect: (criterion: SortCriterion) => void
}

type SortCriterion = 'priority' | 'name' | 'occupancy' | 'severity' | 'location'
...
const sortOptions: SortOption[] = [
  { value: 'priority', label: 'Por: Prioridade', description: 'Ordena por nível de urgência' },
  ...
  { value: 'severity', label: 'Por: Severidade', description: 'Ordena por nível de severidade' },
  { value: 'location', label: 'Por: Localização', description: 'Ordena por bairro' },
]

export default function ShelterFilterSortModal({ open, onClose, onSelect }: ShelterFilterSortModalProps) {
  const [selected, setSelected] = useState<SortCriterion>('priority')
